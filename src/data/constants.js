export const Bio = {
  name: "Manmeet Singh",
  roles: ["Software Engineer", "Backend Engineer", "Full Stack Developer"],
  description: "A passionate software engineer specializing in backend engineering, full stack development, distributed computing, and scalable applications. Possesses a deep enthusiasm for designing and implementing robust, efficient, and reliable software solutions.",
  github: "https://github.com/Manmeet-123",
  resume: "https://drive.google.com/file/d/1AmTpj5gONn1zW8iVyjNtZkP7YcAAR9HG/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/manmeet-singh-365047127/",
}

export const skills = [
  {
    title: 'Frontend',
    skills: [
      {
        name: 'React Js',
        image: 'data:image/svg+xml;base64,',
      },
      {
        name: 'Redux',
        image: 'https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg',
      },
      {
        name: 'HTML',
        image: 'https://www.w3.org/html/logo/badge/html5-badge-h-solo.png',
      },
      {
        name: 'CSS',
        image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png',
      },
      {
        name: 'JavaScript',
        image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png',
      },
    ],
  },
  {
    title: 'Backend',
    skills: [
       {
        name: 'Java',
        image: 'https://logos-world.net/wp-content/uploads/2022/07/Java-Logo.png',
      },
       {
        name: 'Spring Boot',
        image: 'https://pbs.twimg.com/profile_images/1235868806079057921/fTL08u_H_400x400.png',
      },
      {
        name: 'Node Js',
        image: 'https://nodejs.org/static/images/logo.svg',
      },
      {
        name: 'Express Js',
        image: 'data:image/png;base64,',
      },
      {
        name: 'Graph Ql',
        image: 'https://www.vectorlogo.zone/logos/graphql/graphql-icon.svg',
      },
      {
        name: 'Python',
        image: 'https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg',
      },
      {
        name: 'MySQL',
        image: 'https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg',
      },
      {
        name: 'Postgresql',
        image: 'https://www.postgresql.org/media/img/about/press/elephant.png',
      },
      {
        name: 'MongoDB',
        image: 'https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg',
      },
    ],
  },
  {
    title: 'Others',
    skills: [
      {
        name: 'Git',
        image: 'https://camo.githubusercontent.com/fbfcb9e3dc648adc93bef37c718db16c52f617ad055a26de6dc3c21865c3321d/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6769742d73636d2f6769742d73636d2d69636f6e2e737667',
      },
      {
        name: 'GitHub',
        image: 'https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png',
      },
      {
        name: 'Docker',
        image: 'https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg',
      },
      {
        name: 'Kubernetes',
        image: 'https://pbs.twimg.com/card_img/1676965612583788546/zRSqSX3h?format=png&name=small',
      },
      {
        name: 'Jenkins',
        image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e9/Jenkins_logo.svg/1200px-Jenkins_logo.svg.png',
      },
      {
        name: 'VS Code',
        image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519',
      },
      {
        name: 'Postman',
        image: 'https://camo.githubusercontent.com/93b32389bf746009ca2370de7fe06c3b5146f4c99d99df65994f9ced0ba41685/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f676574706f73746d616e2f676574706f73746d616e2d69636f6e2e737667',
      },
       {
        name: 'AWS',
        image: 'https://is1-ssl.mzstatic.com/image/thumb/Purple126/v4/1a/33/61/1a336147-5565-79c8-a1ec-513647befed7/AppIcon-0-1x_U007emarketing-0-7-0-85-220.png/1200x630wa.png'    
      },
    ],
  },
]

export const experiences = [
  {
    id: 0,
    img: 'https://innovate.uncg.edu/wp-content/themes/yootheme/cache/collaboratory_logomark_standard-3-afd74e0c.png',
    role: 'Research And Development Intern',
    company: 'Collaboratory',
    date: 'March 2023 - Present',
    desc: 'Working on designing and building a web application for automating the process of extracting the product metrics',
    skills: ['ReactJS', 'Redux', 'NodeJs', 'Go', 'JavaScript', 'PostGreSql'],
    },
  {
    id: 1,
    img: 'https://www.unitedhealthgroup.com/content/dam/UHG/icons/SocialMediaCards/Artical-Card_Optum-orange_1062x550.png',
    role: 'Software Engineer',
    company: 'Optum (UnitedHealth Group)',
    date: 'July 2020 - August 2022',
    desc: 'Worked on building a web based application using camunda. Build the ETL pipeline using Kafka.',
    skills: ['Java', 'SpringBoot', 'MySql', 'DB2', 'Kubernetes', 'Docker'],
  },
  {
    id: 2,
    img: 'https://upload.wikimedia.org/wikipedia/en/b/b5/DTU%2C_Delhi_official_logo.png',
    role: 'Undergraduate Research Assistant',
    company: 'Delhi College of Engineering',
    date: 'May 2019 - May 2020',
    desc: "Worked on bulding a Hybrid Recommender System using Artificial Bee Colony and graph database. Published the work in Springer lecture notes.",
    skills: ['SQL','Python','Graph Database', 'Machine Learning'],
  },
  {
    id: 3,
    img: 'https://www.unitedhealthgroup.com/content/dam/UHG/icons/SocialMediaCards/Artical-Card_Optum-orange_1062x550.png',
    role: 'Software Engineer Intern',
    company: 'Optum (UnitedHealth Group)',
    date: 'June 2019 - August 2019',
    desc: "Worked on building a conversational Interface using the Google Dialogflow V2 REST APIs and built the connection using Webhook. Technologies used: AngularJS, NodeJS, REST APIs, MongoDB",
    skills: ['AngularJS', 'NodeJS', 'MongoDB', 'Express', 'Node Js'],
    }
]

export const education = [
  {
    id: 0,
    img: 'https://www.asu.edu/modules/composer/webspark-module-asu_brand/node_modules/@asu-design-system/component-header/dist/assets/img/arizona-state-university-logo-vertical.png',
    school: 'Arizona State University',
    date: 'August 2022 - May 2024',
    desc: "Graduate student at Ira Fulton School of Engineering, ASU majoring in Software Engineering.",
    degree: 'Master of Science, Computer Software Engineering',
  },
  {
    id: 1,
    img: 'https://upload.wikimedia.org/wikipedia/en/b/b5/DTU%2C_Delhi_official_logo.png',
    school: 'Delhi College of Engineering',
    date: 'August 2016 - May 2020',
    desc: "Completed Bachelor of Technology with major in Computer Engineering",
    degree: 'Bachelor of Technology, Computer Engineering',
  },
]

export const projects = [
  {
    id: 0,
    title: 'TextAppenderAWS',
    date: 'Apr 2023 - May 2023',
    description: "The goal of this project is to develop a text appender application that appends text data to a file, stores it in Amazon S3, and makes an entry in DynamoDB, leveraging EC2 instances. The application will facilitate efficient storage and retrieval of appended text data, ensuring data integrity and scalability.",
    image: 'https://miro.medium.com/v2/resize:fit:1358/0*REegl5CQvWCaSAIX.gif',
    tags: ['AWS', 'EC2', 'Node Js', 'Lambda', 'S3', 'Amplify'],
    category: 'Full Stack',
    github: 'https://github.com/Manmeet-123/TextAppenderAWS',
   
  },
  {
    id: 1,
    title: 'Digit Recognition System',
    date: 'June 2019 - July 2019',
    description: "The Digit Recognition System is Used to recognise the handwritten digits",
    image: 'https://miro.medium.com/v2/resize:fit:372/1*AO2rIhzRYzFVQlFLx9DM9A.png',
    tags: ['Python', 'Jupyter', 'Tensorflow'],
    category: 'Back End',
    github: 'https://github.com/Manmeet-123/DigitRecognitionSystem'
  },
  {
    id: 2,
    title: 'Hand Detection System',
    date: 'June 2019 - June 2019',
    description: "Hand Detection System to count number of fingers raised in front of the camera",
    image: 'https://techcrunch.com/wp-content/uploads/2019/08/handtracking_shot.png',
    tags: ['Python', 'Jupyter', 'Tensorflow'],
    category: 'Back End',
    github: 'https://github.com/Manmeet-123/HandDetectionSystem',
  },
  {
    id: 3,
    title: 'Todo Web App',
    date: 'May 2019 - July 2019',
    description: "A web based Todolist application for keeping a list of activities you want to do",
    image: 'https://camo.githubusercontent.',
    tags: ['React Js', 'Local Storage', 'HTML', 'Node JS', 'MongoDB'],
    category: 'web app',
    github: 'https://github.com/Manmeet-123/Todolists',
  },
   {
    id: 4,
    title: 'Personal Portfolio',
    date: 'July 2023 - Present',
    description: "A github webpage to showcase the portfolio. Built as a react app.",
    image: 'https://www.fusionacademy.com/wp-content/uploads/2022/08/portfolio.jpg',
    tags: ['React Js', 'HTML', 'Node JS', 'Javascript'],
    category: 'web app',
    github: 'https://github.com/Manmeet-123/Manmeet-123.github.io',
  },

];

